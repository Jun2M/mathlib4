...
def IsAcyclic : Prop := ∀ ⦃v : V⦄ (c : HasDart.Walk G v v), ¬c.IsCycle
...
private lemma _root_.HasDart.Walk.exists_mem_contains_edges_of_directed (Hs : Set <| SimpleGraph V)
    (hHs : Hs.Nonempty) (h_dir : DirectedOn (· ≤ ·) Hs) {u v : V} (p : HasDart.Walk (sSup Hs) u v) :
...
lemma IsAcyclic.mem_support_of_ne_mem_support_of_adj_of_isPath (hG : G.IsAcyclic) {u v w : V}
    {p : HasDart.Walk G u v} {q : HasDart.Walk G u w} (hp : p.IsPath) (hq : q.IsPath)
    (hadj : G.Adj v w) (hv : v ∉ q.support) : w ∈ p.support := by
...
lemma IsAcyclic.ne_mem_support_of_support_of_adj_of_isPath (hG : G.IsAcyclic) {u v w : V}
    {p : HasDart.Walk G u v} {q : HasDart.Walk G u w} (hp : p.IsPath) (hq : q.IsPath)
    (hadj : G.Adj v w) (hw : w ∈ p.support) : v ∉ q.support := by
...
lemma IsAcyclic.path_concat (hG : G.IsAcyclic) {u v w : V} {p : HasDart.Walk G u v}
    {q : HasDart.Walk G u w} (hp : p.IsPath) (hq : q.IsPath) (hadj : G.Adj v w)
    (hv : v ∈ q.support) : q = p.concat hadj := by
...
theorem isTree_iff_existsUnique_path :
    G.IsTree ↔ Nonempty V ∧ ∀ v w : V, ∃! p : HasDart.Walk G v w, p.IsPath := by
  classical
...
lemma IsTree.existsUnique_path (hG : G.IsTree) : ∀ v w, ∃! p : HasDart.Walk G v w, p.IsPath :=
  (isTree_iff_existsUnique_path.1 hG).2

theorem IsAcyclic.isPath_iff_isChain (hG : G.IsAcyclic) {v w : V} (p : HasDart.Walk G v w) :
...
theorem IsAcyclic.isPath_iff_isTrail (hG : G.IsAcyclic) {v w : V} (p : HasDart.Walk G v w) :
    p.IsPath ↔ p.IsTrail :=
...
theorem isAcyclic_add_edge_iff_of_not_reachable (x y : V) (hxy : ¬ G.Reachable x y) :
  ...
  intro u c hc
  let c' : HasDart.Walk G u u := Walk.transfer c G (fun e he ↦ by
...
lemma reachable_eq_of_maximal_isAcyclic (F : SimpleGraph V)
  ...
  contrapose! h
  obtain ⟨⟨p : HasDart.Walk G u v⟩, h : ¬ F.Reachable u v⟩ := h
