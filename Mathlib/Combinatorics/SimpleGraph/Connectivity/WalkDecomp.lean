...
lemma takeUntil_cons {v' : V} {p : Walk G v' v} (hwp : w ∈ p.support) (h : u ≠ w)
    (hadj : G.Adj u v') :
    (p.cons hadj).takeUntil w (List.mem_of_mem_tail (by exact hwp))
      = (p.takeUntil w hwp).cons hadj := by
...
def rotate {u v : V} (c : Walk G v v) (h : u ∈ c.support) : Walk G u u :=
  (c.dropUntil u h).append (c.takeUntil u h)

@[simp]
theorem support_rotate {u v : V} (c : Walk G v v) (h : u ∈ c.support) :
...
@[simp]
theorem mem_support_rotate_iff (c : Walk G v v) (h : u ∈ c.support) :
    w ∈ (c.rotate h).support ↔ w ∈ c.support := by
  grind [rotate, take_spec, mem_support_append_iff]

theorem rotate_darts {u v : V} (c : Walk G v v) (h : u ∈ c.support) :
    (c.rotate h).darts ~r c.darts := by
  simp only [rotate, darts_append]
  apply List.IsRotated.trans List.isRotated_append
  rw [← darts_append, take_spec]

theorem rotate_edges {u v : V} (c : Walk G v v) (h : u ∈ c.support) :
