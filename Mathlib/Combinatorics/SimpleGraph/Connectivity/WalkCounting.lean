...
theorem set_walk_self_length_zero_eq (u : V) :
    {p : HasDart.Walk G u u | p.length = 0} = {Walk.nil} := by
  simp

theorem set_walk_length_zero_eq_of_ne {u v : V} (h : u ≠ v) :
    {p : HasDart.Walk G u v | p.length = 0} = ∅ := by
  ext p
  simp only [Set.mem_setOf_eq, Set.mem_empty_iff_false, iff_false]
  exact fun h' => absurd (Walk.eq_of_length_eq_zero h') h

theorem set_walk_length_succ_eq (u v : V) (n : ℕ) :
    {p : HasDart.Walk G u v | p.length = n.succ} =
      ⋃ (w : V) (h : G.Adj u w), Walk.cons h '' {p' : HasDart.Walk G w v | p'.length = n} := by
  ext p
  cases p with
  | nil => simp [eq_comm]
  | cons huw pwv =>
...
def walkLengthTwoEquivCommonNeighbors (u v : V) :
    {p : HasDart.Walk G u v // p.length = 2} ≃ G.commonNeighbors u v where
  toFun p := ⟨p.val.snd, match p with
    | ⟨.cons _ (.cons _ .nil), _⟩ => ⟨‹G.Adj u _›, ‹G.Adj _ v›.symm⟩⟩
  invFun w := ⟨(dart.toWalk w.prop.1).concat w.prop.2.symm, rfl⟩
...
def finsetWalkLength (n : ℕ) (u v : V) : Finset (HasDart.Walk G u v) :=
  match n with
  | 0 =>
    if h : u = v then by
      subst u
...
theorem coe_finsetWalkLength_eq (n : ℕ) (u v : V) :
    (G.finsetWalkLength n u v : Set (HasDart.Walk G u v)) =
      {p : HasDart.Walk G u v | p.length = n} := by
...
variable {G} in
theorem mem_finsetWalkLength_iff {n : ℕ} {u v : V} {p : HasDart.Walk G u v} :
...
def finsetWalkLengthLT (n : ℕ) (u v : V) : Finset (HasDart.Walk G u v) :=
...
theorem coe_finsetWalkLengthLT_eq (n : ℕ) (u v : V) :
    (G.finsetWalkLengthLT n u v : Set (HasDart.Walk G u v)) =
      {p : HasDart.Walk G u v | p.length < n} := by
  ext p
  simp [finsetWalkLengthLT, mem_finsetWalkLength_iff]

variable {G} in
theorem mem_finsetWalkLengthLT_iff {n : ℕ} {u v : V} {p : HasDart.Walk G u v} :
    p ∈ G.finsetWalkLengthLT n u v ↔ p.length < n :=
  Set.ext_iff.mp (G.coe_finsetWalkLengthLT_eq n u v) p

instance fintypeSetWalkLength (u v : V) (n : ℕ) : Fintype {p : HasDart.Walk G u v | p.length = n} :=
...
instance fintypeSubtypeWalkLength (u v : V) (n : ℕ) :
    Fintype {p : HasDart.Walk G u v // p.length = n} :=
  fintypeSetWalkLength G u v n

theorem set_walk_length_toFinset_eq (n : ℕ) (u v : V) :
    {p : HasDart.Walk G u v | p.length = n}.toFinset = G.finsetWalkLength n u v := by
...
theorem card_set_walk_length_eq (u v : V) (n : ℕ) :
    Fintype.card {p : HasDart.Walk G u v | p.length = n} = #(G.finsetWalkLength n u v) :=
...
instance fintypeSetPathLength (u v : V) (n : ℕ) :
    Fintype {p : HasDart.Walk G u v | p.IsPath ∧ p.length = n} :=
...
instance Path.instFintype {u v : V} : Fintype (G.Path u v) where
  elems := (univ (α := { p : HasDart.Walk G u v | p.IsPath ∧ p.length < Fintype.card V })).map
