...
lemma _root_.HasDart.Walk.IsCycle.isCycles_spanningCoe_toSubgraph {u : V} {p : HasDart.Walk G u u}
...
set_option backward.isDefEq.respectTransparency false in
lemma _root_.HasDart.Walk.IsPath.isCycles_spanningCoe_toSubgraph_sup_edge {u v}
    {p : HasDart.Walk G u v} (hp : p.IsPath) (h : u ≠ v) (hs : s(v, u) ∉ p.edges) :
...
lemma _root_.HasDart.Walk.IsCycle.adj_toSubgraph_iff_of_isCycles [LocallyFinite G] {u}
    {p : HasDart.Walk G u u} (hp : p.IsCycle) (hcyc : G.IsCycles) (hv : v ∈ p.toSubgraph.verts) :
...
lemma IsCycles.snd_of_mem_support_of_isPath_of_adj [Finite V] {v w w' : V} (hcyc : G.IsCycles)
    (p : HasDart.Walk G v w) (hw : w ≠ w') (hw' : w' ∈ p.support) (hp : p.IsPath)
...
private lemma IsCycles.reachable_sdiff_toSubgraph_spanningCoe_aux [Finite V] {v w : V}
    (hcyc : G.IsCycles) (p : HasDart.Walk G v w) (hp : p.IsPath) :
  ...
  use (((hcyc.reachable_sdiff_toSubgraph_spanningCoe_aux
    (p.cons hw'2.symm) hp'p).some).mapLe hle).append (dart.toWalk this)
...
lemma IsCycles.reachable_sdiff_toSubgraph_spanningCoe [Finite V] {v w : V} (hcyc : G.IsCycles)
    (p : HasDart.Walk G v w) (hp : p.IsPath) : (G \ p.toSubgraph.spanningCoe).Reachable w v := by
...
lemma IsCycles.reachable_deleteEdges [Finite V] (hadj : G.Adj v w)
  ...
  have : fromEdgeSet {s(v, w)} = (dart.toWalk hadj).toSubgraph.spanningCoe := by
  ...
  exact this ▸ (hcyc.reachable_sdiff_toSubgraph_spanningCoe (dart.toWalk hadj)
...
lemma IsCycles.exists_cycle_toSubgraph_verts_eq_connectedComponentSupp [Finite V]
    ...
    ∃ (p : HasDart.Walk G v v), p.IsCycle ∧ p.toSubgraph.verts = c.supp := by
  classical
  obtain ⟨w, hw⟩ := hn
